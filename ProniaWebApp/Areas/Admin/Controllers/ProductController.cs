...
namespace ProniaWebApp.Areas.Admin.Controllers
{
	...
	public class ProductController : Controller
	{
		...
		public async Task<IActionResult> Update(UpdateProductVM updateProductVM)
		{
			...
			Product existProduct = await _context.Products.Include(p=>p.ProductsTags).ThenInclude(pt=>pt.Tag).Include(p => p.ProductImages).Where(p => p.Id == updateProductVM.Id).FirstOrDefaultAsync();
			...
			//existProduct.ProductsTags.Clear();
			...
			//         existProduct.ProductImages.RemoveAll();
			//existProduct.ProductImages.RemoveAll(p=>updateProductVM.ImageIds.Contains(p.Id));
